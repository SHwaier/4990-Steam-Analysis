	...
	return (
		<main className="min-h-screen bg-gradient-to-b from-black via-slate-950 to-black">
			<div className="container mx-auto px-4 py-12">
				{/* Back Button */}
				<Link href="/" className="mb-8 inline-flex items-center text-gray-400 hover:text-white transition-colors">
					← Back to Home
				</Link>
				...
				{sentimentStats && (
					<div className="mb-12 grid gap-6 md:grid-cols-4">
						<SpotlightCard>
							...
							<div className="mt-4">
								<div className="text-5xl font-bold text-white">{getSentimentPercentage()}%</div>
						...
						<SpotlightCard>
							...
							<div className="mt-4">
								<div className="text-5xl font-bold text-green-400">{sentimentStats.overall.positiveCount}</div>
						...
						<SpotlightCard>
							...
							<div className="mt-4">
								<div className="text-5xl font-bold text-yellow-400">{sentimentStats.overall.neutralCount}</div>
								<div className="text-gray-400">Reviews</div>
							</div>
						</SpotlightCard>
						...
						<SpotlightCard>
							...
							<div className="mt-4">
								<div className="text-5xl font-bold text-red-400">{sentimentStats.overall.negativeCount}</div>
								<div className="text-gray-400">Reviews</div>
							</div>
						</SpotlightCard>
					</div>
				...
				<div className="mb-12 grid gap-6 lg:grid-cols-2">
					...
					<Card>
						...
						<div className="mt-4 text-sm text-gray-400" dangerouslySetInnerHTML={{ __html: game.short_description }} />
						...
						<div className="mt-6 flex gap-3">
							<a href={`https://store.steampowered.com/app/${appid}`} target="_blank" rel="noopener noreferrer" className="inline-flex rounded-full bg-blue-600 px-6 py-3 font-semibold text-white transition-all hover:bg-blue-700">
								View on Steam →
							</a>
							<WishlistButton appid={parseInt(appid)} name={game.name} header_image={game.header_image} variant="button" />
						</div>
					</Card>
									...
									<div key={item.date} className="flex items-center justify-between">
										...
										<div className="flex items-center gap-2">
											<div className="h-2 w-32 rounded-full bg-gray-700 overflow-hidden">
												<div
													className={`h-full ${item.averageScore > 1 ? "bg-green-500" : item.averageScore < -1 ? "bg-red-500" : "bg-yellow-500"}`}
													style={{
														width: `${Math.min(100, Math.max(0, ((item.averageScore + 5) / 10) * 100))}%`,
													}}
												/>
											</div>
											<span className="text-sm text-white w-16 text-right">{item.count} reviews</span>
										</div>
									</div>
								))}
							</div>
				...
				<Tabs defaultValue="reviews" className="w-full">
					...
					<TabsContent value="reviews">
						<div className="mb-6 flex items-center justify-between flex-wrap gap-4">
							<h2 className="text-3xl font-bold text-white">Recent Reviews with Sentiment Analysis</h2>
							...
							<div className="flex gap-4 flex-wrap">
								...
								<Select label="Sort by Sentiment" value={sortBySentiment} onChange={(e) => setSortBySentiment(e.target.value)}>
									<option value="default">Default</option>
									<option value="positive">Most Positive</option>
									<option value="negative">Most Negative</option>
								</Select>

								<Select label="Sort by Playtime" value={sortByPlaytime} onChange={(e) => setSortByPlaytime(e.target.value)}>
									<option value="default">Default</option>
						...
						<div className="space-y-4">
							{reviews.length > 0 ? (
								<>
											...
											<div className="flex items-start justify-between">
												<div className="flex-1">
													<div className="mb-2 flex items-center gap-2 flex-wrap">
														<Badge variant={review.sentimentLabel}>
															{review.sentimentLabel.toUpperCase()} ({review.sentiment.score}/100)
														</Badge>
														<Badge variant={review.voted_up ? "positive" : "negative"}>{review.voted_up ? "👍 Recommended" : "👎 Not Recommended"}</Badge>
														<span className="text-sm text-gray-400">{new Date(review.timestamp_created * 1000).toLocaleDateString()}</span>
													</div>
									...
									{!hasMore && reviews.length > 0 && <div className="text-center py-8 text-gray-400">No more reviews to load</div>}
								</>
							) : (
