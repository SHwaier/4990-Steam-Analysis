...
export function analyzeReview(review: SteamReview): ReviewWithSentiment {
  ...
  const effortSignal = Math.min(
    Math.log(1 + text.length) / Math.log(1 + 3000),
    1
  );
  ...
  const helpfulSignal = Math.min(
    Math.log(1 + (review.votes_up || 0)) / Math.log(1 + 500),
    1
  );
  ...
  const hasMultipleSentences =
    (text.match(/[.!?]+/g) || []).length >= 2 ? 0.25 : 0;
  const hasEmoji = /[\u{1F300}-\u{1FAFF}]|[\u{2600}-\u{27BF}]|[❤♥👍👎]/u.test(
    text
  )
    ? 0.25
    : 0;
  const toneSignal = Math.min(
    hasExclamation + hasCaps + hasMultipleSentences + hasEmoji,
    1
  );

  // ── Combine into final score ──
  let score: number = 50;
  const voteScore = review.voted_up ? 10 : -10;

  // Text Contribution
  const textContrib = textAligned
    ? textMagnitude * 25 // agreeing text → boost
    : textMagnitude * -15; // contradicting text → pull

  const effortContrib = effortSignal * 10;
  const playtimeContrib = playtimeSignal * 8;
  const helpfulContrib = helpfulSignal * 5;
  const toneContrib = toneSignal * 4;

  const metadataTotal =
    effortContrib + playtimeContrib + helpfulContrib + toneContrib;

  if (review.voted_up) {
    score = 50 + voteScore + textContrib + metadataTotal;
  } else {
    score = 50 + voteScore - textContrib - metadataTotal;
  }
...
export function getSentimentLabel(score: number): 'positive' | 'negative' {
...
export function calculateSentimentStats(
  ...
  const timeline = Array.from(reviewsByDate.entries())
    .map(([date, dateReviews]) => ({
      date,
      averageScore:
        dateReviews.reduce((sum, r) => sum + r.sentiment.score, 0) /
        dateReviews.length,
...
// ─── UI Helpers ────────────────────────────────────────────────────────
export function getSentimentColor(label: 'positive' | 'negative'): string {
  switch (label) {
    case 'positive':
...
export function getSentimentBgColor(label: 'positive' | 'negative'): string {
  switch (label) {
    case 'positive':
...
export function getSentimentEmoji(label: 'positive' | 'negative'): string {
  switch (label) {
    case 'positive':
      return '😊';
    case 'negative':
